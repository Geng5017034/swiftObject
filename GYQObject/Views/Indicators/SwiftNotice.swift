...
class SwiftNotice: NSObject {
    ...
    class func noticeOnStatusBar(_ text: String, autoClear: Bool, autoClearTime: Int) {
        ...
        UIView.animate(withDuration: 0.3, animations: {
            view.frame = CGRect(origin: destPoint, size: view.frame.size)
                ...
                if autoClear {
                    ...
                    let timer = Timer(timeInterval: TimeInterval(autoClearTime), target: self, selector: selector, userInfo: window, repeats: false)
    ...
    class func wait(_ imageNames: Array<UIImage> = Array<UIImage>(), timeInterval: Int = 0) {
    ...
    class func showText(_ text: String,autoClearTime: Float) {
        let window = UIWindow()
        window.backgroundColor = UIColor.clear
        let mainView = UIView()
        mainView.layer.cornerRadius = 12
    ...
    class func showNoticeWithText(_ type: NoticeType,text: String, autoClear: Bool, autoClearTime: Float) {
        let frame = CGRect(x: 0, y: 0, width: 95, height: 95)
        let window = UIWindow()
        window.backgroundColor = UIColor.clear
        let mainView = UIView()
        ...
        checkmarkView.frame = CGRect(x: (95-34)/2, y: 16, width: 34, height: 34)
        mainView.addSubview(checkmarkView)

        let label = UILabel(frame: CGRect(x: 2, y: 61-5, width: 91, height: 16*2))
        label.font = UIFont.systemFont(ofSize: 12)
    ...
    // fix https://github.com/johnlui/SwiftNotice/issues/2
    class func hideNotice(_ sender: Timer) {
        if let window = sender.userInfo as? UIWindow {
    ...
    // just for iOS 8
    class func getRealCenter() -> CGPoint {
